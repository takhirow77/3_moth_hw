...
async def end_register_user(message:types.Message, state:FSMContext):
   ...
   user_date = (message.from_user.id, data['first_name'], data['last_name'], data['phone'], data['age'], time.strftime("%Y-%m-%d %H:%M:%S"))
   cursor.execute("INSERT INTO users (id, first_name, last_name, phone, age, created) VALUES(?,?,?,?,?,?)",user_date)
   connection.commit()
